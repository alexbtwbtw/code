...
# ── Caller IP auto-detection ──────────────────────────────────────────────────
# Used to restrict SSH access to the machine running terraform apply.
# Override with var.deploy_ssh_cidr in terraform.tfvars if needed.
data "http" "my_ip" {
  url = "https://ifconfig.me"
}

locals {
  deploy_ssh_cidr = var.deploy_ssh_cidr != null ? var.deploy_ssh_cidr : "${trimspace(data.http.my_ip.response_body)}/32"
}
...
module "networking" {
  ...
  region          = var.region
  deploy_ssh_cidr = local.deploy_ssh_cidr
